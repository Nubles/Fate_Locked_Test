...
import {
  ...
  ExternalLink, Unlock, Lock, Compass, ChevronDown, ChevronsUp, AlertCircle, BookOpen, ScrollText
} from 'lucide-react';
...
const TABS = [
  ...
  { id: 'WORLD', label: 'World', icon: Map, color: 'text-emerald-400', border: 'border-emerald-500' },
  { id: 'ACTIVITIES', label: 'Activities', icon: Swords, color: 'text-red-400', border: 'border-red-500' },
  { id: 'UTILITY', label: 'Utility', icon: Package, color: 'text-amber-400', border: 'border-amber-500' },
...
const UnlockCard: React.FC<UnlockCardProps> = ({
  ...
  return (
    <div
        ...
        className={`
            relative flex items-center gap-3 p-2 rounded-lg border transition-all duration-200 group min-h-[50px]
            ${isUnlocked
...
export const Dashboard: React.FC = () => {
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
                {SKILLS_LIST.map(skill => {
                    ...
                    const canLevel = isUnlocked && level < cap;

                    const canOmniUpgrade = specialKeys > 0 && tier < 10;
                    const isStuck = isUnlocked && level >= cap && tier < 10;
                    ...
                    const handleMainClick = () => {
                        if (canUnlockStart) {
                             ...
                             handleLevelUp(skill);
                        } else if (isStuck && canOmniUpgrade) {
                             handleSpecialUnlock(TableType.SKILLS, skill);
                        }
                    };

                    const isMainActionable = canUnlockStart || canLevel || (isStuck && canOmniUpgrade);

                    const tierColorText = tier > 0
                        ? (tier >= 10 ? 'text-yellow-400'
                        : tier >= 9 ? 'text-fuchsia-400'
                        : tier >= 7 ? 'text-red-400'
                        : tier >= 6 ? 'text-cyan-400'
                        : 'text-[#fbbf24]')
                        : 'text-gray-500';
                    ...
                    return (
                        <div
                           ...
                           className={`
                                ...
                                ${canUnlockStart || (isStuck && canOmniUpgrade) ? 'ring-1 ring-purple-500 hover:bg-purple-900/10 cursor-pointer' : ''}
                        ...
                        >
                            ...
                            {isUnlocked && canOmniUpgrade && (
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${isStuck
                                            ? 'bg-purple-600 text-white animate-pulse border-purple-400 shadow-[0_0_8px_rgba(168,85,247,0.6)]'
                            ...
                            <div className="flex items-center gap-2 mb-2 w-full pointer-events-none">
                                ...
                                <div className="min-w-0 flex-1">
                                    ...
                                    <div className="text-[9px] text-gray-400 font-mono leading-none mt-0.5">
                                        {isUnlocked ? `Lvl ${level}/${cap}` : 'Locked'}
                                    </div>
                                </div>
                            </div>

                            {/* Segmented Progress Bar */}
                            <div className="flex gap-4 w-full h-1.5 mt-auto bg-black/50 rounded-sm overflow-hidden border border-white/5 pointer-events-none">
                            ...
                            {isMainActionable && (
                                <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none z-10">
                                    <span className="text-[9px] font-bold bg-black/80 px-2 py-0.5 rounded text-white border border-white/20 shadow-lg">
                                        {canUnlockStart ? 'UNLOCK' : (canLevel ? 'LEVEL UP' : 'UNLOCK TIER')}
                                    </span>
                                </div>
                            )}
                        </div>
  ...
  const renderWorldTab = () => (
      <div className="space-y-6 h-full overflow-y-auto pr-2 custom-scrollbar pb-10">
          {/* Regions */}
          <div className="space-y-4">
               <h3 className="text-emerald-400 font-bold text-sm uppercase tracking-wide border-b border-white/10 pb-2">Regions</h3>
  ...
  return (
    <div className="bg-osrs-panel border border-osrs-border rounded-lg shadow-lg flex flex-col h-full overflow-hidden relative">
      ...
      <div className="flex flex-col md:flex-row border-b border-white/5 bg-[#161616] shrink-0">
          ...
          <div className="p-2 border-l border-white/5 relative bg-[#161616]">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 w-3.5 h-3.5" />
              <input
                type="text"
                placeholder="Filter unlocks..."
                className="bg-black/30 border border-white/10 rounded-full py-1 pl-8 pr-3 text-xs text-white placeholder-gray-600 focus:outline-none focus:border-white/20 w-40 transition-all focus:w-56"
                value={searchQuery}
