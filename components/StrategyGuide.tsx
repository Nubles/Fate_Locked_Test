...
import { CheckCircle, XCircle, Lock, Map, BookOpen, AlertCircle, Compass, Target, Search, ScrollText, Filter, Pin, SlidersHorizontal, Check } from 'lucide-react';

// Create efficient lookup sets for items that ACTUALLY require a key to unlock.
const ROOT_UNLOCKS = {
...
// Helpers to analyze state
const analyzeRequirement = (req: ContentRequirement, unlocks: any) => {
    // 1. Regions
    const missingRegions = req.regions.filter(r => {
        // Base case: Misthalin is always unlocked
        if (r === 'Misthalin' || MISTHALIN_AREAS.includes(r)) return false;

        // Direct match: The specific area is unlocked
        if (unlocks.regions.includes(r)) return false;

        // Group match: If 'r' is a broad region (e.g. 'Kandarin'), check if any sub-area is unlocked.
        // This heuristic assumes that unlocking a chunk gives access to the region's main content for that tier.
        if (REGION_GROUPS[r]) {
    ...
    const totalReqs = req.regions.length + Object.keys(req.skills).length + (req.quests?.length || 0) + 1; // +1 for category lock
...
export const StrategyGuide: React.FC<{ onClose: () => void }> = ({ onClose }) => {
    ...
    const [activeTab, setActiveTab] = useState<'PLAYABLE' | 'PLANNER'>('PLAYABLE');
    ...
    const allContent = useMemo<Record<string, ContentRequirement>>(() => {
        ...
        Object.values(DIARY_DATA).forEach(d => {
            ...
            database[d.id] = {
                ...
                regions: [...d.requiredRegions], // Ensure main region is checked
                skills: d.skills,
                quests: d.quests,
        ...
        // 3. Strategy Database (Manual overrides/additions)
        // This takes precedence for descriptions/custom logic
        Object.entries(STRATEGY_DATABASE).forEach(([key, val]) => {
            // Check if already completed (for things we track like Bosses/Minigames that are unlocks)
            // If it's a Quest/Diary already in unlocks, skip
    ...
    // Planner content sorted by Completion Percentage (Descending)
            ...
            <div className="bg-[#121212] border border-gray-700 w-full max-w-5xl h-[85vh] rounded-xl flex flex-col shadow-2xl overflow-hidden">
                ...
                <div className="bg-[#1a1a1a] p-4 border-b border-gray-700 flex justify-between items-center shrink-0">
                    ...
                    <div className="flex bg-black/40 rounded-lg p-1 border border-white/10">
                        ...
                        >
                            Available ({playableContent.length})
                        </button>
                        <button
                                        ...
                                        return (
                                            <div key={content.uniqueId} className="bg-[#1a1a1a] border border-emerald-500/30 p-4 rounded-lg shadow-[0_0_15px_rgba(16,185,129,0.05)] hover:bg-[#202020] transition-colors group relative flex flex-col h-full">
                                                <div className="flex justify-between items-start mb-2">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            ...
                                                            className={`p-1 rounded transition-colors ${isPinned ? 'text-blue-400 bg-blue-900/20' : 'text-gray-600 hover:text-gray-300'}`}
                                                            title={isPinned ? "Unpin Goal" : "Pin as Goal"}
                                                        >
                                                ...
                                                {content.id !== content.uniqueId && (
                                                    <p className="text-[10px] text-gray-600 mb-1 font-mono uppercase tracking-wider">{content.id}</p>
                                                )}
                                                ...
                                                <div className="pt-3 border-t border-white/5 flex gap-2 flex-wrap">
                                                    {content.regions.map(r => (
                                                        <span key={r} className="text-[9px] flex items-center gap-1 text-gray-400 bg-black/40 px-1.5 py-0.5 rounded border border-white/5">
                                                            <Map size={10} /> {r}
                                                        </span>
                                                    ))}
                                                    {Object.keys(content.skills).map(s => (
                                                        <span key={s} className="text-[9px] flex items-center gap-1 text-gray-400 bg-black/40 px-1.5 py-0.5 rounded border border-white/5">
                                                            <BookOpen size={10} /> {s}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        return (
                                            <div key={content.uniqueId} className="bg-[#161616] border border-white/5 rounded-lg p-4 flex flex-col lg:flex-row gap-6 relative overflow-hidden group hover:border-white/10 transition-colors">
                                                ...
                                                <div className="absolute bottom-0 left-0 h-1 bg-white/5 w-full">
                                                    <div className={`h-full ${progressColor} transition-all duration-500`} style={{ width: `${completionPercent}%` }}></div>
                                                </div>
                                                ...
                                                <div className="lg:w-1/3 shrink-0 flex flex-col">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex flex-col gap-1">
                                                             ...
                                                             {content.id !== content.uniqueId && (
                                                                 <span className="text-[10px] text-gray-500 font-mono uppercase tracking-wider">{content.id}</span>
                                                             )}
                                                        </div>
                                                        <button
                                                            onClick={() => togglePin(content.uniqueId)}
                                                            className={`p-1.5 rounded transition-colors ${isPinned ? 'text-blue-400 bg-blue-900/20' : 'text-gray-600 hover:text-gray-300'}`}
                                                            title={isPinned ? "Unpin Goal" : "Pin as Goal"}
                                                        >
                                                    ...
                                                    <div className="mt-auto flex flex-col gap-1">
                                                        {!isCategoryUnlocked && (
                                                            <div className="flex items-center gap-2 text-red-400 text-xs font-bold bg-red-900/10 p-1.5 rounded border border-red-500/20">
                                                                <Lock size={12} />
                                                                <span>LOCKED CATEGORY (Requires Key)</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex-1 flex flex-col justify-center border-t lg:border-t-0 lg:border-l border-white/5 pt-4 lg:pt-0 lg:pl-6">
                                                    <div className="text-[9px] font-bold uppercase tracking-widest text-gray-600 mb-3 flex items-center gap-2">
                                                        <Target size={10} /> Requirements Breakdown
                                                    </div>
                                                    ...
                                                    <div className="flex flex-wrap items-start gap-2">
                                                        ...
                                                        {content.regions.map((region) => {
                                                            ...
                                                            return (
                                                                <div key={region} className={`flex items-center gap-1.5 px-2 py-1.5 rounded border text-xs font-mono transition-colors ${isMissing ? 'bg-red-900/20 border-red-500/30 text-red-400' : 'bg-green-900/10 border-green-500/20 text-green-500/70'}`}>
                                                                    <Map size={12} />
                                                                    <span>{region}</span>
                                                                    {isMissing ? <Lock size={10} /> : <Check size={10} />}
                                                                </div>
                                                            )
                                                        })}
                                                        ...
                                                        {Object.entries(content.skills).map(([skill, level]) => {
                                                            ...
                                                            return (
                                                                <div key={skill} className={`flex items-center gap-1.5 px-2 py-1.5 rounded border text-xs font-mono transition-colors ${isMissing ? 'bg-red-900/20 border-red-500/30 text-red-400' : 'bg-green-900/10 border-green-500/20 text-green-500/70'}`}>
                                                                    <BookOpen size={12} />
                                                                    <span>{skill} {level}</span>
                                                                    {isLocked ? <Lock size={10} /> : isMissing ? <span className="text-[9px] bg-red-500/20 px-1 rounded">{skillStatus?.currentLevel}</span> : <Check size={10} />}
                                                                </div>
                                                            )
                                                        })}
                                                        ...
                                                        {content.quests && content.quests.map(q => {
                                                            ...
                                                            return (
                                                                <div key={q} className={`flex items-center gap-1.5 px-2 py-1.5 rounded border text-xs font-mono transition-colors ${isMissing ? 'bg-red-900/20 border-red-500/30 text-red-400' : 'bg-green-900/10 border-green-500/20 text-green-500/70'}`}>
                                                                    <ScrollText size={12} />
                                                                    {q}
                                                                    {isMissing ? <Lock size={10} /> : <Check size={10} />}
                                                                </div>
