...
const ToastNotification = () => {
  ...
  useEffect(() => {
    ...
    if (undoableTypes.includes(lastEvent.type)) {
      ...
      if (lastEvent.type === 'LEVEL_UP') msg = 'Level Up';
      ...
      <header className="bg-[#1e1e1e] border-b border-white/10 sticky top-0 z-50 shadow-xl backdrop-blur-md bg-opacity-95">
        <div className="max-w-[1600px] mx-auto px-4 py-2 flex flex-col xl:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0 self-start xl:self-center">
            <div className="w-9 h-9 bg-gradient-to-br from-amber-600 to-amber-900 rounded-lg flex items-center justify-center border border-amber-500/50 shadow-inner">
              <span className="text-xl drop-shadow-md">🗝️</span>
            </div>
            <div>
              <h1 className="text-base font-black text-gray-100 tracking-tight uppercase leading-none">Fate-Locked OSRS</h1>
              <p className="text-[9px] text-gray-500 font-mono mt-0.5">V1.1 COMMAND CENTER</p>
            </div>
          </div>

          <div className="flex flex-col md:flex-row items-center gap-3 md:gap-6 bg-black/20 p-1.5 pr-4 rounded-xl border border-white/5 w-full xl:w-auto">
            <div className="w-full md:w-48 lg:w-64 px-2">
               ...
               <div className="h-2.5 bg-black/50 rounded-full overflow-hidden border border-white/10 relative">
            ...
            <div className="flex items-center gap-3 justify-center">
                <div className="flex items-center gap-2 px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                   ...
                   <span className="font-bold text-amber-100 text-base leading-none">{keys}</span>
                </div>
                <div className={`flex items-center gap-2 px-3 py-1 rounded-lg border transition-colors ${specialKeys > 0 ? 'bg-purple-500/20 border-purple-500/40 shadow-[0_0_10px_rgba(168,85,247,0.2)]' : 'bg-white/5 border-white/10 opacity-50'}`}>
                   <Sparkles className={`w-3.5 h-3.5 ${specialKeys > 0 ? 'text-purple-400 animate-pulse' : 'text-gray-500'}`} />
                   <span className={`font-bold text-base leading-none ${specialKeys > 0 ? 'text-purple-200' : 'text-gray-500'}`}>{specialKeys}</span>
                </div>
                {chaosKeys > 0 && (
                  <div className="flex items-center gap-2 px-3 py-1 bg-red-500/20 border border-red-500/40 rounded-lg animate-in fade-in slide-in-from-right-4">
                     <Dna className="w-3.5 h-3.5 text-red-400 animate-pulse" />
                     <span className="font-bold text-red-100 text-base leading-none">{chaosKeys}</span>
                  </div>
                )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0 self-end xl:self-center">
             ...
             <button
                ...
                className={`group px-3 py-1.5 rounded-lg border font-bold text-[10px] uppercase tracking-wider flex items-center gap-2 transition-all shadow-lg ${activeBuff !== 'NONE' ? activeBuff === 'GREED' ? 'bg-amber-900/40 border-amber-500 text-amber-300' : 'bg-blue-900/40 border-blue-500 text-blue-300' : 'bg-[#252525] border-purple-500/30 text-purple-300 hover:bg-purple-900/20'}`}
             >
             ...
             <div className="flex items-center bg-[#252525] border border-white/10 rounded-lg p-0.5 gap-0.5">
                 <button onClick={() => setShowOracle(true)} className="p-1.5 text-gray-400 hover:text-white hover:bg-white/5 rounded transition-colors" title="Oracle (Ctrl+K)"><Search size={14} /></button>
                 <button onClick={() => setShowStrategy(true)} className="p-1.5 text-gray-400 hover:text-emerald-400 hover:bg-white/5 rounded transition-colors" title="Strategy Guide"><Compass size={14} /></button>
                 <button onClick={() => setShowShare(true)} className="p-1.5 text-gray-400 hover:text-pink-400 hover:bg-white/5 rounded transition-colors" title="Share"><Share2 size={14} /></button>
                 <button onClick={() => setShowStats(true)} className="p-1.5 text-gray-400 hover:text-blue-400 hover:bg-white/5 rounded transition-colors" title="Stats"><BarChart3 size={14} /></button>
                 <button onClick={() => setShowReference(true)} className="p-1.5 text-gray-400 hover:text-yellow-400 hover:bg-white/5 rounded transition-colors" title="Rules"><HelpCircle size={14} /></button>
             </div>

             <div className="flex items-center bg-[#252525] border border-white/10 rounded-lg p-0.5 gap-0.5">
                 <button onClick={toggleAnimations} className={`p-1.5 rounded transition-colors ${animationsEnabled ? 'text-green-400' : 'text-gray-500'}`} title="Animations">
                    {animationsEnabled ? <PlayCircle size={14} /> : <PauseCircle size={14} />}
                 </button>
                 <button onClick={() => fileInputRef.current?.click()} className="p-1.5 text-gray-500 hover:text-white hover:bg-white/5 rounded" title="Import"><Upload size={14} /></button>
                 <button onClick={() => {
                 ...
                 }} className="p-1.5 text-gray-500 hover:text-white hover:bg-white/5 rounded" title="Export"><Download size={14} /></button>
                 <button onClick={() => { if(window.confirm("Reset?")) resetGame(); }} className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-white/5 rounded" title="Reset"><RotateCcw size={14} /></button>
             </div>
          </div>
        </div>
      </header>
...
const ControlPanel = ({ onShowChronicle }: { onShowChronicle: () => void }) => {
  const [activeTab, setActiveTab] = useState<'FARM' | 'SPEND' | 'LOG'>('SPEND');
